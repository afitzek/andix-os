...
void pmm_dump() {
	...
	pmm_debug("Memory Area:    START   |     END   | TYPE");
	pmm_debug("===============================================");
	list* pos;
	list* next;
	list_for_each_safe(pos, next, physical_mem_list)
	{
		...
		if(area) {
			pmm_debug("Memory Area: 0x%x | 0x%x | %c",
		...
		for (i = 0; i < pmm_bitmap_size / 4; i++) {
			if (secure_phy_mem[i]) { // There is one bit set!
				for (j = 0; j < 32; j++) {
					if (secure_phy_mem[i] & (1 << j)) { // Find the set bit!
						if (k == 0) {
							uint32_t saddr = (uint32_t)(((uint8_t*) phys_start_addr + (i * 32 + j) * 4096));
							//pmm_debug("saddr: 0x%x", saddr);
							uint32_t padding = (uint32_t) (align - ((uint32_t) saddr & (align - 1)));
